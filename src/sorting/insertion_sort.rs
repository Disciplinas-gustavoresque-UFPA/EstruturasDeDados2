...
mod tests {
    ...
    fn test_empty() {
        ...
        assert_eq!(insertion_sort(&values), vec![]);
    }

    #[test]
    fn test_single_element() {
        let values = vec![42];
        assert_eq!(insertion_sort(&values), vec![42]);
    }

    #[test]
    fn test_already_sorted() {
        let values = vec![1, 2, 3, 4, 5];
        assert_eq!(insertion_sort(&values), vec![1, 2, 3, 4, 5]);
    }
    ...
    fn test_unsorted_mixed_values() {
        ...
        assert_eq!(insertion_sort(&values), vec![1, 2, 3, 4, 5, 6, 7, 8, 9]);
    }

    #[test]
    fn test_negative_numbers() {
        let values = vec![-3, 0, -10, 7, -1, 2];
        assert_eq!(insertion_sort(&values), vec![-10, -3, -1, 0, 2, 7]);
    }

    #[test]
    fn test_duplicate_values() {
        let values = vec![4, 2, 2, 1, 3, 4, 1];
        assert_eq!(insertion_sort(&values), vec![1, 1, 2, 2, 3, 4, 4]);
    }

    #[test]
    fn test_all_equal_values() {
        let values = vec![7, 7, 7, 7, 7];
        assert_eq!(insertion_sort(&values), vec![7, 7, 7, 7, 7]);
    }

    #[test]
    fn test_integer_boundaries() {
        let values = vec![i32::MAX, i32::MIN, 0, -1, 1];
        assert_eq!(insertion_sort(&values), vec![i32::MIN, -1, 0, 1, i32::MAX]);
    }
    ...
    fn test_characters() {
        ...
        assert_eq!(insertion_sort(&values), vec!['a', 'b', 'c', 'd']);
    }
}